...
def send_digest_email(hours: int = 24, top_n: int = 10) -> dict:
    try:
        ...
        # After successful send, mark all emailed digests as sent
        # so they never appear in future digest emails again
        repo = Repository()
        sent_ids = [article.digest_id for article in result.articles]
        marked = repo.mark_digests_as_sent(sent_ids)
        logger.info(f"Marked {marked} digests as sent")
        ...
        logger.info(f"No new digests to send: {e}")
        return {
            "success": False,
            "error": str(e)
        }
